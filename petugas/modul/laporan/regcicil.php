...
$siswa = mysqli_query($koneksi, "SELECT * from tagihan inner join siswa on tagihan.NIS=siswa.NIS INNER join kelas on siswa.id_kls=kelas.id_kelas 
  INNER join bayar on tagihan.id_tagihan=bayar.id_tag join orangtua ON orangtua.NIS=siswa.NIS where kategoripem LIKE '%" . $Semester . "%' AND tagihan.status='Cicilan' order by id_tagihan DESC ");
...
?>
  ...
  <tr>
    ...
    <td><button class="btn btn-danger" data-toggle="modal" data-target="#hapus_karyawan<?= $pro['id_tagihan']; ?>"><i class="fa fa-trash"></i></button>
      <div class="modal fade bd-example-modal-lg" id="hapus_karyawan<?= $pro['id_tagihan']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
